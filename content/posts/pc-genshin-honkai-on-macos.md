...
目前网上暂时没找到其它运行 PC 崩铁的方法。PlayCover 用过一段时间，太难用，不适合键鼠。

出于对一些项目的保护目的，有些工具不方便直接提供，你可以直接从上下文中找到信息（没有解谜，能一眼看出来），但我不会直接写出来。

__⚠️崩铁存在封号案例，风险自负⚠️__

__⚠️崩铁存在封号案例，风险自负⚠️__

__⚠️崩铁存在封号案例，风险自负⚠️__

嗯，原神这么多年了暂时没见过封号案例，但是毕竟不能算是正规途径，还是需要风险自负。
...
你需要[下载 Command Line Tools](https://developer.apple.com/download/all/?q=Command%20Line%20Tools)，要求至少达到 15 Beta 版本，但是最高 15 Beta 2 版本（因为 Beta 3 和 Beta 4 都有[数起编译失败的报告](https://developer.apple.com/forums/thread/732940)）。同时你可以在这个页面提前搜索[下载 Game Porting Toolkit](https://developer.apple.com/download/all/?q=Game%20Porting%20Toolkit)，之后用得着。
...
你还需要安装 x86\_64 的 Homebrew，因为苹果也通过它来分发 Game Porting Toolkit 的组件。

可能需要注意的是，如果你已经安装 arm64 版的 Homebrew，它默认应该安装在 `/opt/homebrew` 下。x86\_64 的 Homebrew 默认安装在 `/usr/local/Homebrew` 下，它们是通过目录来隔离的，请确保你的 arm64 Homebrew 没有安装到其它目录，以防混淆。接下来我也会使用完整路径，以便于区分。
...
从这里开始，请尽量不要退出当前 Shell 或打开新的终端、Shell，因为新的 Shell 默认还是 arm64 环境。如果不慎退出了当前 Shell，你可以再次输入上述命令重新进入 x86\_64 环境；如果你忘记了当前的终端是否正处于 x86_64 环境下，可以使用 `uname -m` 命令来检查当前的架构。
...
[下载 Game Porting Toolkit](https://developer.apple.com/download/all/?q=Game%20Porting%20Toolkit) 完后将其挂载到文件树（双击打开就行，别告诉我不会挂载），默认情况下会挂载到 `/Volumes/Game Porting Toolkit-1.0` 目录下，将 lib 目录复制到我们用 Homebrew 编译的 game-porting-toolkit 目录下。
...
那么到这里 Game Porting Toolkit 已经安装完毕了。如果你进到它在 Homebrew 的目录里观察一下，会发现它其实就是一个 Wine，不过它是苹果修改过后的 Wine。

## Wine 配置

后面都已经不需要在 x86_64 环境下了。

Wine 的中文字体有必要装一下，不影响游戏本体，但是轻的有遇到其它程序无法显示汉字的情况，严重有原神中打开「版本热点」和「云原神」时卡死整个游戏进程导致无法关闭窗口、以及游戏关闭时也会卡死只能强杀进程的问题。
...
Wine 默认模拟的操作系统是 Windows 7，最好再执行一下 `winecfg.exe`，或者通过 Winetricks 打开 wine 配置程序，将 Windows 版本设置为 Windows 10。设置界面是图形化的，不难操作。
...
这里加上 grep 是为了防止大量的调试日志输出被 macOS 系统收集，这会导致在游戏退出后 macOS 系统试图弹一个很长的错误对话框，导致进程卡死。
...
避免本博客被官方用关键字检索到，这里用 base64 加密了。

出于对项目的保护目的，这里不提供绕过反作弊的工具，你可以想办法从上下文中推断出信息来；或者想办法联系上我，我私聊提供链接。

是不是有股黑产卖病毒的味道，我特么也担心是病毒。不过代码是开源的，你可以自己分析源码构建一遍，如果放心的话也可以直接用 Release 构建。还是那句话，风险自负。
...
原神基本完全可用。原神 3.8 开始似乎删除了反作弊模块，所以可以直接用 Game Porting Toolkit 打开，不然也需要其它补丁。

原神目前发现唯一的瑕疵就是草地贴图似乎有点问题，经常不显示或者闪烁。但是不影响游戏本身内容（该在草地上燃烧还是能燃烧）。

崩铁使用了腾讯的 AntiCheatExpert 反作弊模块（相比起来，原神和崩三都是自研的），这是个内核模块，就是它导致了 Wine 无法直接运行崩铁。所以上文采用了特殊的绕过方法。

崩铁启动的时候因为这个绕过反作弊工具的限制，只能直接启动游戏本体，无法经过启动器启动。这大多数情况下都不会有问题，但是遇到版本更新的时候，可能需要手动打开启动器去下载新版内容。这点还好，一般一个版本只需要一次。

崩铁启动的时候会弹窗提醒有封号风险，这是绕过工具的提醒。这是在代码中写死的，你可以通过修改源码自己重新编译绕过。

崩铁启动时音响会炸两下，不太清楚原因，但是游戏中声音是正常的。

还是提醒一遍，在崩铁 1.2 版本以前有大量因为使用了这个绕过方法导致短期中期封号的报告，<strong>风险自负。</strong>1.2 版本目前过了大约一周都暂时没见到封号，但是我无法做担保，__风险自负！__

还有退出时经常遇到本体已经关闭，但是 wineserver 还没关的情况，这时候可能需要手动去 `killall wineserver` 一遍，暂时没有好的解决方法。这个可能不是游戏的原因。
